...
import core.records.Proceedings;
 ...
 *
 */
public class ProceedingsFactory extends AbstractFactory {
    ...
    public static BibTeXEntity createRecord(Map<Attributes, List<String>> attributes, String key) {
        ...
        if (checkRequiredAttributes(attributes, Proceedings.requiredAttributes)) {

            deleteUnnecessaryAttributes(attributes, Proceedings.acceptableAttributes);
            return new Proceedings(attributes, key);
        } else {
            throw new IncorrectFields("Proceedings not have all required attributes:" + key);
        }
    }
}
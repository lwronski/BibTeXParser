...
import core.records.MastersThesis;
 ...
 *
 */
public class MasterThesisFactory extends AbstractFactory {
    ...
    public static BibTeXEntity createRecord(Map<Attributes, List<String>> attributes, String key) {
        ...
        if (checkRequiredAttributes(attributes, MastersThesis.requiredAttributes)) {

            deleteUnnecessaryAttributes(attributes, MastersThesis.acceptableAttributes);
            return new MastersThesis(attributes, key);
        } else {
            throw new IncorrectFields("MastersThesis not have all required attributes:" + key);
        }
    }
}
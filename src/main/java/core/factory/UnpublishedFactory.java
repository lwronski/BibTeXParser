...
import core.records.Unpublished;
 ...
 *
 */
public class UnpublishedFactory extends AbstractFactory {
    ...
    public static BibTeXEntity createRecord(Map<Attributes, List<String>> attributes, String key) {
        ...
        if (checkRequiredAttributes(attributes, Unpublished.requiredAttributes)) {

            deleteUnnecessaryAttributes(attributes, Unpublished.acceptableAttributes);
            return new Unpublished(attributes, key);
        } else {
            throw new IncorrectFields("Unpublished not have all required attributes:" + key);
        }
    }
}
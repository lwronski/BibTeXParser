 ...
 *
 */
public class InBookFactory extends AbstractFactory {
    ...
    public static BibTeXEntity createRecord(Map<Attributes, List<String>> attributes, String key) {
        ...
        if (checkRequiredAttributes(attributes, InBook.requiredAttributes)) {
            deleteUnnecessaryAttributes(attributes, InBook.acceptableAttributes);
            return new InBook(attributes, key);
        } else {
